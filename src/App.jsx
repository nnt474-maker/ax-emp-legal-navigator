...
import {
  ...
  TYPE_META,
  TYPE_ORDER,
} from "./data/legalNavigatorData.js";
...
function toAxisList(item) {
  return item.axisList?.length ? item.axisList : [item.axis].filter(Boolean);
}
  ...
  for (let pass = 0; pass < 6; pass += 1) {
    TYPE_ORDER.forEach((type) => {
      ...
      currentLayer.sort((a, b) => {
        ...
        const bNeighbors = (adjacency.get(b.id) ?? [])
          .map((id) => indexMap.get(id))
          .filter((value) => Number.isFinite(value));
...
function AxisToggle({ axis, active, count, onClick }) {
  const meta = AXIS_META[axis];
  return (
    <button
      type="button"
      className={`axis-toggle ${active ? "is-active" : ""}`}
      onClick={onClick}
    >
      <span className="axis-toggle__top">
        <span className="axis-toggle__code">{meta.short}</span>
        <span className="axis-toggle__count">{count}</span>
      </span>
      <span className="axis-toggle__label">{meta.label}</span>
    </button>
  );
}
...
function App() {
  ...
  const axisCounts = useMemo(
    () =>
      AXIS_FILTER_ORDER.reduce((acc, axis) => {
        acc[axis] =
          axis === "ALL"
            ? RAW_NODES.length
            : RAW_NODES.filter((node) => toAxisList(node).includes(axis)).length;
        return acc;
      }, {}),
    []
  );

  const axisEdgeCounts = useMemo(
    () =>
      AXIS_FILTER_ORDER.reduce((acc, axis) => {
        acc[axis] =
          axis === "ALL"
            ? RAW_EDGES.length
            : RAW_EDGES.filter((edge) => toAxisList(edge).includes(axis)).length;
        return acc;
      }, {}),
    []
  );

  const axisFilteredNodes = useMemo(() => {
    if (activeAxis === "ALL") return RAW_NODES;
    return RAW_NODES.filter((node) => toAxisList(node).includes(activeAxis));
  }, [activeAxis]);

  const typeCounts = useMemo(
    () =>
      TYPE_ORDER.reduce((acc, type) => {
        acc[type] = axisFilteredNodes.filter((node) => node.type === type).length;
        return acc;
      }, {}),
    [axisFilteredNodes]
  );
  ...
  const filteredNodes = useMemo(() => {
    ...
    return axisFilteredNodes.filter((node) => {
  ...
  }, [axisFilteredNodes, search, selectedTypes]);

  const visibleNodeIds = useMemo(() => new Set(filteredNodes.map((node) => node.id)), [filteredNodes]);

  const filteredEdges = useMemo(
    () =>
      RAW_EDGES.filter((edge) => {
        if (!visibleNodeIds.has(edge.source) || !visibleNodeIds.has(edge.target)) return false;
        if (activeAxis === "ALL") return true;
        return toAxisList(edge).includes(activeAxis);
      }),
    [activeAxis, visibleNodeIds]
  );
  ...
  const connectedItems = useMemo(() => {
    ...
    return filteredEdges
      ...
      .sort((a, b) => {
        const typeGap = TYPE_ORDER.indexOf(a.other.type) - TYPE_ORDER.indexOf(b.other.type);
  ...
  const overlapCount = useMemo(
    () => axisFilteredNodes.filter((node) => toAxisList(node).length > 1).length,
    [axisFilteredNodes]
  );

  const activeAxisMeta = AXIS_META[activeAxis];
  const selectedNodeAxes = selectedNode ? toAxisList(selectedNode) : [];
    ...
    <div className="app-shell">
      <aside className="sidebar sidebar--left">
        <div className="brand-block">
          ...
          <h1 className="brand-block__title">{activeAxisMeta.title}</h1>
          <p className="brand-block__desc">{activeAxisMeta.description}</p>
        </div>

        <section className="panel-card">
          <div className="panel-card__title">Nhánh quan hệ</div>
          <div className="axis-toggle-grid">
            {AXIS_FILTER_ORDER.map((axis) => (
              <AxisToggle
                key={axis}
                axis={axis}
        ...
        <section className="panel-card">
          ...
          <input
            ...
            placeholder="Tìm theo mã, tên mục hoặc từ khóa..."
          />
        </section>
        ...
        <section className="panel-card panel-card--muted">
          ...
          <ul className="help-list">
            <li>Chọn nhánh quan hệ ở trên để lọc riêng AX-EMP, AX-SUP hoặc xem toàn bộ bản đồ hợp nhất.</li>
            <li>Chọn một ô trên sơ đồ để xem mô tả ngắn, trục áp dụng và các mục liên quan trực tiếp.</li>
            <li>Mặc định app giữ nguyên toàn cảnh. Chỉ khi bật chế độ ẩn mục không liên quan thì sơ đồ mới thu gọn theo nội dung đang xem.</li>
          </ul>
        </section>
          ...
          <div className="legend-group">
            {TYPE_ORDER.map((type) => (
              <div key={type} className="legend-row">
                <span className="legend-dot" style={{ background: TYPE_META[type].color }} />
      ...
      <main className="canvas-shell">
        <div className="canvas-shell__stats">
          <StatPill label="Nhánh" value={activeAxisMeta.short} />
          <StatPill label="Tổng mục" value={axisCounts[activeAxis]} />
          <StatPill label="Đang hiện" value={flowData.nodes.length} />
          <StatPill label="Tổng liên kết" value={axisEdgeCounts[activeAxis]} />
          <StatPill label="Đang hiện LK" value={flowData.edges.length} />
          <StatPill label="Overlap" value={overlapCount} />
          {TYPE_ORDER.map((type) => (
            <StatPill key={type} label={type} value={typeCounts[type]} />
          ))}
        </div>
        ...
        <div className="canvas-shell__hint">
          <strong>Mẹo:</strong> nếu đang xem bản đồ gộp, hãy chọn nhánh <strong>AX-EMP</strong> hoặc <strong>AX-SUP</strong> trước khi đi sâu. Khi cần kiểm tra giao điểm giữa hai nhánh, quay về <strong>Tất cả</strong> rồi chọn một node đang có overlap.
        </div>
      </main>
        ...
        <div className="details-header">
          ...
          <p className="details-header__subtitle">
            {selectedNode
              ...
              : "Chọn một ô trong sơ đồ để xem mô tả ngắn, trục áp dụng và chuyển nhanh sang nội dung liên quan."}
          </p>
        </div>
        ...
        {selectedNode ? (
          <div className="details-stack">
            <section className="detail-card">
              ...
              <div className="detail-copy">{selectedNode.note || "Mục này đang được hiển thị để hỗ trợ tra cứu nhanh trong Legal Navigator."}</div>
            ...
            <section className="detail-card detail-card--compact">
              ...
              <div className="kv-row">
                <span className="kv-row__key">Trục chính</span>
                <span className="kv-row__value">{getAxisLabel(selectedNode.primaryAxis || selectedNode.axis)}</span>
              </div>
              <div className="kv-row">
                <span className="kv-row__key">Trục áp dụng</span>
                <div className="chip-list">
                  {selectedNodeAxes.map((axis) => (
                    <span
                      key={axis}
                      className={`meta-chip ${axis === (selectedNode.primaryAxis || selectedNode.axis) ? "is-primary" : ""}`}
                    >
                      {axis}
                    </span>
                  ))}
                    ...
                    return (
                      ...
                      >
                        <div className="linked-item__top">
                          ...
                          <span className="linked-item__relation" style={{ color: relationMeta.color }}>
                            {relationMeta.label}
                          </span>
                        </div>

                        <div className="linked-item__title">{other.nameVi}</div>
                        <div className="linked-item__axes">{toAxisList(other).map(getAxisLabel).join(" · ")}</div>

                        {edge.note ? <div className="linked-item__note">{edge.note}</div> : null}
                      </button>
                    );
                  })
                ) : (
          ...
          <div className="empty-state">
            ...
            <ul className="help-list">
              <li>Chọn một Luật để xem các hồ sơ và quy trình liên quan.</li>
              <li>Chuyển sang AX-SUP để tra cơ quan giám sát, báo cáo, cấp phép và thanh tra.</li>
              <li>Quay về Tất cả để soi các node giao nhau giữa lao động và giám sát.</li>
            </ul>
          </div>
        )}
      </aside>
...
export default function LegalNavigatorApp() {
  return <App />;
}
